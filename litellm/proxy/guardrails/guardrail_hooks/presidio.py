...
# Longest run of text the streaming output-masking path will hold without a
# sentence boundary before force-flushing it through Presidio. Bounds worst-case
# TTFT on punctuation-free output at the cost of possibly splitting a multi-word
# entity that straddles the cap.
_PRESIDIO_STREAM_MAX_BUFFER = 400
...
class _OPTIONAL_PresidioPIIMasking(CustomGuardrail):
    ...
    def _mask_flush_index(text: str) -> int:
        ...
        Past ``_PRESIDIO_STREAM_MAX_BUFFER`` with no such boundary we force-flush
        at the last whitespace to bound TTFT."""
        boundaries = tuple(
            i + 1
            for i in range(len(text))
            if text[i] == "\n"
            or (text[i] in ".!?" and (i + 1 == len(text) or text[i + 1].isspace()))
        )
        if boundaries:
            return boundaries[-1]
        if len(text) > _PRESIDIO_STREAM_MAX_BUFFER:
            whitespace = tuple(i + 1 for i in range(len(text)) if text[i].isspace())
            return whitespace[-1] if whitespace else len(text)
        return 0
