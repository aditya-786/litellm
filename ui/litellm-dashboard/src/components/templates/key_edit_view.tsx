...
export function KeyEditView({
  ...
  const [tagRateLimits, setTagRateLimits] = useState<TagRateLimitEntry[]>(
    tagLimitsToRows(keyData.metadata?.tag_rpm_limit, keyData.metadata?.tag_tpm_limit),
  );
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      // Always send the current per-tag limit maps so removing every row
      // clears the stored limits ({} overwrites the metadata field).
      const { tag_rpm_limit, tag_tpm_limit } = tagRowsToLimits(tagRateLimits);
      values.tag_rpm_limit = tag_rpm_limit;
      values.tag_tpm_limit = tag_tpm_limit;
    ...
    <Form form={form} onFinish={handleSubmit} initialValues={initialValues} layout="vertical">
      ...
      <Form.Item
        label={
          <span>
            Per-Tag Rate Limits{" "}
            <Tooltip title="Scope rate limits to a request tag so each tag (e.g. a cell or group) gets its own RPM/TPM counter. Requests without a matching tag fall back to the key-level limits.">
              <InfoCircleOutlined style={{ marginLeft: "4px" }} />
            </Tooltip>
          </span>
        }
